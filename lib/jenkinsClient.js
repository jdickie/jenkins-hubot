...
JenkinsClient.prototype.getStatusOneJob = function(callback, params) {
  try {
    var self = this;
    if (_.every(["codebase", "environment", "name"], _.partial(_.has, params))) {
      var path = config.get("Jobs." + params.environment + "." + params.codebase) + "/" + params.name;
      self._client.job.get(path, callback);
    } else {
      callback("Didn't get all required fields");
    }
  } catch (e) {
    console.log(e);
    callback("Error");
