...
UpdateFile.prototype.putEnviromentInfoFile = function(callback, params) {
  var self = this;

  try {
      testReport.createReportHeader(function(err, header) {
        if (err) {
          callback(err);
        }
        self.sendToS3(callback, header, config.get('aws.envInfoFilename'));
